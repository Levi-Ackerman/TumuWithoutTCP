    ...
    public static double getForce3() {
        ...
        for (JiePin jiePin : jiePins) {
            if (jiePin.n == 1) {
                ...
                if (kexi >= 60) {
                    f3 += midu * jiePin.freq * jiePin.freq * lineLength * lineLength * (1 - 4.4 * c / jiePin.freq - 1.1 * c * c / jiePin.freq / jiePin.freq);
                } else if (kexi >= 17 && kexi <= 60) {
                    f3 += midu * jiePin.freq * jiePin.freq * lineLength * lineLength * (1.03 - 6.33 * c / jiePin.freq - 1.58 * c * c / jiePin.freq / jiePin.freq);
                } else {
                    f3 += midu * jiePin.freq * jiePin.freq * lineLength * lineLength * (0.882 - 85 * c * c / jiePin.freq / jiePin.freq);
                }
            } else {
